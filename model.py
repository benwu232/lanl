...
class WaveNet(nn.Module):
    def __init__(self, pars):
        ...
        self.conv1 = Conv1D1x1(1, self.n_blk_res, bias=self.bias, weight_norm=self.weight_norm, init_type=self.init_type)
        ...
        for layer in range(self.n_layers):
            ...
            wnl = WaveNetBlk(self.n_blk_res, self.n_blk_skip, self.kernel_size, dilation=dilation, weight_norm=self.weight_norm, init_type=self.init_type)
...
def set_optimizer(model, optim_pars):
    if optim_pars['type'] == 'SGD':
        ...
        optimizer = Adam(model.parameters(), lr=optim_pars['lr'], betas=(optim_pars['beta1'], optim_pars['beta2']), eps=optim_pars['epsilon'], weight_decay=optim_pars['l2_scale'])
